...
void Converter::ReadSkinData()
{
	for (UINT i = 0; i < scene->mNumMeshes; i++)
	{
		...
		if (aiMesh->HasBones() == false) return;
...
void Converter::ReadMaterialData()
{
	...
	{
		aiMaterial* srcMaetrial = scene->mMaterials[i];

		if (FoundMaterialData(srcMaetrial) == false)
			continue;

		asMaterial* material = new asMaterial();
		material->Name = srcMaetrial->GetName().C_Str();

		aiColor3D color;

		srcMaetrial->Get(AI_MATKEY_COLOR_AMBIENT, color);
		material->Ambient = Color(color.r, color.g, color.b, 1.0f);

		srcMaetrial->Get(AI_MATKEY_COLOR_DIFFUSE, color);
		material->Diffuse = Color(color.r, color.g, color.b, 1.0f);

		srcMaetrial->Get(AI_MATKEY_COLOR_SPECULAR, color);
		material->Specular = Color(color.r, color.g, color.b, 1.0f);

		srcMaetrial->Get(AI_MATKEY_SHININESS, material->Specular.a);

		srcMaetrial->Get(AI_MATKEY_COLOR_EMISSIVE, color);
		material->Emissive = Color(color.r, color.g, color.b, 1.0f);

		aiString file;

		srcMaetrial->GetTexture(aiTextureType_DIFFUSE, 0, &file);
		material->DiffuseFile = file.C_Str();

		srcMaetrial->GetTexture(aiTextureType_SPECULAR, 0, &file);
		material->SpecularFile = file.C_Str();

		srcMaetrial->GetTexture(aiTextureType_NORMALS, 0, &file);
